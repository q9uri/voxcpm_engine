...
class VoxCPM:
    ...
    def _generate(self, 
        ...
        try:
            ...
            len_phone_text = len(phone_text)

            if len_phone_text < 5:
                phone_prompt_text = prompt_text.replace("_", "")
                phone_prompt_text = phone_prompt_text.replace("!", "")
                phone_prompt_text = phone_prompt_text.replace("?", "")
                phone_prompt_text = phone_prompt_text.replace("'", "")
                prompt_text = phone_prompt_text.replace("/", "")
            """
            ...
            """
            #if len_prompt_text > len_text:
            #    dynamic_max = (len_prompt_text - len_text) * decode_limit_factor + (len_prompt_text - len_text) // 2
            #else:
